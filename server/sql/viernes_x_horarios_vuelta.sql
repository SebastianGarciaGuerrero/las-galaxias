...
-- Esto reparte las horas de la J8 a la J12 con las condiciones que
-- puso Sebastián:
--
--   · Malajax juega una sola vez a las 19:00, y es contra
--     Charchalax (J8).
--   · Motafogo y Violeta Parra juegan a las 19:00 (J10).
--   · Motafogo juega una sola vez a las 20:00, así no le toca
--     enfrentar a Malajax en el horario temprano.
--   · Nadie juega más de 2 veces en el mismo horario.
--
-- Y cierra parejo: los seis quedan con un partido en un horario y
-- dos en cada uno de los otros.
--
...
--
-- ES LA ÚNICA
-- Con las fechas quietas hay 7.776 formas de repartir los horarios
-- de las cinco jornadas. Una sola cumple las cuatro condiciones a la
-- vez, y es esta. Si además se permitiera mover partidos de día no
-- aparece ninguna mejor: con el reparto de acá ya nadie pasa de 2.
--
-- El detalle que la destrabó es el último: mientras Malajax y
-- Motafogo tenían que jugar los dos una sola vez a las 19:00,
-- alguien quedaba con 3 sí o sí —probado sobre los 6 calendarios
-- posibles—. Dejando que Motafogo juegue dos veces temprano y una
-- sola a las 20:00, el cruce Malajax - Motafogo se va a las 22:00 y
-- todo encaja.
--
...
-- y ninguno cambia de día. La J10 mantiene a Vasco - Malajax a las
-- 22:00, del cambiazo que ya se corrió.
--
...
update public.matches m
  ...
  from (values
        ...
        (9,  'Violeta Parra',  'Charchalax',     '2026-10-16 23:00:00+00'),
        (9,  'Malajax',        'Motafogo',       '2026-10-17 01:00:00+00'),
        -- J10, viernes 23-10
        ...
        (11, 'Charchalax',     'Vasco de Gramo', '2026-10-31 01:00:00+00'),
        -- J12, viernes 06-11
...
-- Charchalax abre la J8, Violeta - Motafogo abre la J10 y Malajax -
-- Motafogo cierra la J9.
-- ============================================================
...
-- Las seis filas tienen que decir 1, 2 y 2 en algun orden, y
-- Malajax un 1 en la columna de las 19:00.
-- ============================================================
